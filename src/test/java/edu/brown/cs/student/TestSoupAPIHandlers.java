...
import edu.brown.cs.student.server.ActivityHandler;
import edu.brown.cs.student.server.OrderHandler;
...
public class TestSoupAPIHandlers {
  ...
  public void testAPIOneRecipe() throws IOException {
    ...
    HttpURLConnection clientConnection = tryRequest("order?soupName=Carrot");
    // Get an OK response (the *connection* worked, the *API* provides an error response)
    assertEquals(200, clientConnection.getResponseCode());
  ...
  // Recall that the "throws IOException" doesn't signify anything but acknowledgement to the type
  // checker
  public void testAPITwoRecipies() throws IOException {
    menu.add(
            Soup.buildNoExceptions(
                    "Carrot",
                    Arrays.asList("carrot", "onion", "celery", "garlic", "ginger", "vegetable broth")));
    ...
    menu.add(
            Soup.buildNoExceptions(
                    "Squash",
                    Arrays.asList("squash","cream","salt","pepper", "cheese")));

    HttpURLConnection clientConnection = tryRequest("order?soupName=Carrot");
    // Get an OK response (the *connection* worked, the *API* provides an error response)
    assertEquals(200, clientConnection.getResponseCode());

    HttpURLConnection clientConnection_Squash = tryRequest("order?soupName=Squash");
    assertEquals(200, clientConnection_Squash.getResponseCode());
    ...
    System.out.println(clientConnection_Squash.getInputStream());
    OrderHandler.SoupSuccessResponse response2 =
            moshi
                    .adapter(OrderHandler.SoupSuccessResponse.class)
                    .fromJson(new Buffer().readFrom(clientConnection_Squash.getInputStream()));
    ...
    Soup squash =
            new Soup(
                    ...
                    Arrays.asList("squash","cream","salt","pepper", "cheese"),
                    false);
    Map<String, Object> result2 = (Map<String, Object>) response2.responseMap().get("Squash");
    System.out.println(result2.get("ingredients"));
    assertEquals(squash.getIngredients(), result2.get("ingredients"));
    clientConnection_Squash.disconnect();
  }
}
